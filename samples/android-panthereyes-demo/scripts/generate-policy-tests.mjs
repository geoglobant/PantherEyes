...
for (const env of envs) {
  ...
  const changeSet = await generator.generate({
    ...
    namespace: 'com.panthereyes.policy',
  });
...
if (outputMode === 'changeset') {
  ...
  await writeFile(path.join(outDir, 'android-policy-test-changesets.json'), `${JSON.stringify(all, null, 2)}\n`, 'utf8');
  console.log(`Wrote generated JUnit files and ${outDir}/android-policy-test-changesets.json`);
}
